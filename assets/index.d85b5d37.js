...
`,onmounted(){fetch("api/index.json").then(i=>i.json()).then(i=>{this.post=i})},animate:{fadein:l,focus:z,slidein:Y,slideinup:q,slideinups:I,swirlin:P}});const N="modulepreload",u={},W="/",v=function(e,t){return!t||t.length===0?e():Promise.all(t.map(r=>{if(r=`${W}${r}`,r in u)return;u[r]=!0;const a=r.endsWith(".css"),n=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${r}"]${n}`))return;const s=document.createElement("link");if(s.rel=a?"stylesheet":N,a||(s.as="script",s.crossOrigin=""),s.href=r,document.head.appendChild(s),a)return new Promise((x,y)=>{s.addEventListener("load",x),s.addEventListener("error",y)})})).then(()=>e())};var b={"./posts/dative-v2-alpha.md":()=>v(()=>import("./dative-v2-alpha.32f8e620.js"),[]),"./posts/sveltekit.md":()=>v(()=>import("./sveltekit.a52f2ff4.js"),[])};let h=[];for(const i in b){let e=0;b[i]().then(t=>{let r=t.attributes;h.push(p(m({id:e++},r),{body:t.html}))})}o.extend({template:`<div>
  <div class="mt-12">
...
`,data(){return{contents:h}},use:[S],animate:{fadein:l}});let $=o.extend({template:`<div>
...
`,data(){return{name:"EmekaTheDev"}},animate:{flip:f}}),O=o.extend({template:`<div>
...
`;let T=o.extend({animate:{bubble:E,fadein:l},template:`<div>
...
`,data(){return{social_media:[],content:F,attributes:J}},async onmounted(){let e=await(await fetch("api/index.json")).json();this.set({social_media:e.social_medias})}});var g=new M({el:"#app"}),B=new A({el:"#navbar"}),R=new O({el:"#request"}),C=new T({el:"#service"}),U=new $({el:"#footer"});g.attach([B,R,C,U]);window.app=g;
