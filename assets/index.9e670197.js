...
`,data(){return{contents:h}},use:[M],animate:{fadein:l}});let W=o.extend({template:`<div>
...
</div>`}),Y=o.extend({template:`<nav class="bg-white shadow dark:bg-base-300">
    ...
    >
     <div class="absolute top-2 right-3 md:relative md:top-1">
        ...
        <input
          #search
          type="text"
          class="w-full py-2 pl-10 pr-4 text-gray-700 bg-white border rounded-md dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:outline-none"
          ...
          <ul class="menu bg-gray-900 w-56 absolute z-1000 divide-y divide-gray-50">
            ...
            {{/if}}  
              {{#each filteredNames as item}}
                <li><a href="https://{{item.link}}">{{item.name}}</a></li>
              {{/each}} 
          </ul>
        </div>
      </div>
...
`;let q=o.extend({animate:{bubble:z,fadein:l},template:`<div>
