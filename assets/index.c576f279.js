...
`,onmounted(){fetch("api/index.json").then(i=>i.json()).then(i=>{this.post=i})},animate:{fadein:l,focus:M,slidein:Y,slideinup:I,slideinups:P,swirlin:z}});const W="modulepreload",u={},$="/",b=function(e,t){return!t||t.length===0?e():Promise.all(t.map(r=>{if(r=`${$}${r}`,r in u)return;u[r]=!0;const a=r.endsWith(".css"),n=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${r}"]${n}`))return;const o=document.createElement("link");if(o.rel=a?"stylesheet":W,a||(o.as="script",o.crossOrigin=""),o.href=r,document.head.appendChild(o),a)return new Promise((x,y)=>{o.addEventListener("load",x),o.addEventListener("error",y)})})).then(()=>e())};var v={"./posts/dative-v2-alpha.md":()=>b(()=>import("./dative-v2-alpha.32f8e620.js"),[]),"./posts/sveltekit.md":()=>b(()=>import("./sveltekit.a52f2ff4.js"),[])};let h=[];for(const i in v){let e=0;v[i]().then(t=>{let r=t.attributes;h.push(p(m({id:e++},r),{body:t.html}))})}s.extend({template:`<div>
  <div class="mt-12">
...
`,data(){return{contents:h}},use:[S],animate:{fadein:l}});let q=s.extend({template:`<div>
    <div class="bg-main-600 px-2 py-2 text-main-100 relative top-4 text-center">
        <h1 class="text-main-800">\xA9\uFE0F 2022-{{new Date().getFullYear()}} Made With \u{1F5A4} By Tobithedev</h1>
